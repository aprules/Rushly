...
export default function Leads({ session }) {
  ...
  const SortIcon = ({ col }) => {
    ...
    return <span style={{ color: '#00c896', marginLeft: '165px' }}>{sortAsc ? '↑' : '↓'}</span>;
  };
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div>
            <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a1d2e', letterSpacing: '-0.3px' }}>
              Leads
              {total > 0 && <span style={{ fontSize: '13px', fontWeight: '500', color: '#9094a8', marginLeft: '165px' }}>({total.toLocaleString()})</span>}
            </div>
