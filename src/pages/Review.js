...
export default function Review({ session }) {
  ...
  const fetchReview = useCallback(async () => {
    setLoading(true);
    try {
      let query = supabase.from('review').select('*').order('id', { ascending: false });
  ...
  const handleAtm = async (row) => {
    ...
    try {
      ...
      setRows(prev => prev.map(r => r.id === row.id ? { ...r, atm: true } : r));
    } catch(e) {}
          ...
          {NAV_ITEMS.map(item => (
            <div key={item.path} onClick={() => navigate(item.path)}
              style={{
                ...
                color: item.path === '/review' ? '#fff' : 'rgba(255,255,255,0.65)',
                background: item.path === '/review' ? 'rgba(255,255,255,0.1)' : 'transparent',
      ...
      <div style={{ marginLeft: '200px', flex: 1, padding: '32px', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div>
            ...
            <div style={{ fontSize: '13px', color: '#9094a8', marginTop: '2px' }}>Unmatched orgs — review and add to masterlist</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
          ...
          {schools.length > 0 && (
            <select value={filterSchool} onChange={e => setFilterSchool(e.target.value)}
              style={{ height: '36px', background: '#fff', border: '1px solid #e8eaf0', borderRadius: '7px', padding: '0 12px', fontSize: '13px', color: filterSchool ? '#1a1d2e' : '#9094a8', outline: 'none', cursor: 'pointer' }}>
              <option value="">All schools</option>
              {schools.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          )}
          {(search || filterSchool) && (
            <button onClick={() => { setSearch(''); setFilterSchool(''); }}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#f5f6fa', borderBottom: '1px solid #e8eaf0' }}>
                  {['Company', 'Organization', 'School', 'Add to Masterlist'].map(label => (
                    <th key={label} style={{ padding: '10px 16px', textAlign: 'left', fontWeight: '600', color: '#1a1d2e', fontSize: '12px', whiteSpace: 'nowrap' }}>
                      {label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={4} style={{ padding: '40px', textAlign: 'center', color: '#9094a8', fontSize: '13px' }}>Loading...</td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={4} style={{ padding: '40px', textAlign: 'center', color: '#9094a8', fontSize: '13px' }}>
                    {search || filterSchool ? 'No results match your filters.' : 'No unmatched orgs to review.'}
                  </td></tr>
                ) : (
                  rows.map((row) => (
                    <tr key={row.id} style={{ borderBottom: '1px solid #f0f1f5', transition: 'background 0.1s' }}
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        {row.atm || saved[row.id]
                          ? <span style={{ fontSize: '11px', color: '#00c896', fontWeight: '600', background: '#e8faf5', padding: '3px 10px', borderRadius: '20px' }}>✓ Added</span>
                          : <button onClick={() => handleAtm(row)} disabled={saving[row.id]}
                              style={{
                                background: saving[row.id] ? '#f5f6fa' : '#fff',
                                border: '1px solid #e8eaf0', borderRadius: '6px',
                                padding: '4px 12px', fontSize: '11px', fontWeight: '600',
                                color: '#1a1d2e', cursor: saving[row.id] ? 'not-allowed' : 'pointer',
                                transition: 'border-color 0.15s'
                              }}
                              onMouseEnter={e => { if (!saving[row.id]) e.currentTarget.style.borderColor = '#00c896'; }}
                              onMouseLeave={e => { e.currentTarget.style.borderColor = '#e8eaf0'; }}
                            >
                              {saving[row.id] ? 'Adding...' : '+ Add'}
                            </button>
                        }
                      </td>
                    </tr>
