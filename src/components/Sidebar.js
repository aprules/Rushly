...
export default function Sidebar({ session }) {
  ...
  return (
    <div style={{
      width: '180px', minHeight: '100vh', background: '#405189',
      display: 'flex', flexDirection: 'column', flexShrink: 0,
      position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 10,
      fontFamily: "'DM Sans', Segoe UI, sans-serif"
    }}>
      {/* Logo */}
      <div style={{ padding: '16px', borderBottom: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center' }}
        onClick={() => navigate('/dashboard')}>
        <img src={rushlyLogo} alt="Rushly" style={{ height: '28px', width: 'auto', objectFit: 'contain' }} />
      </div>
          ...
          return (
            <div key={section.label} style={{ marginBottom: '2px' }}>
              ...
              <div
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 8px 4px', cursor: 'pointer', borderRadius: '4px' }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
              >
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: '600' }}>
                  {section.label}
                ...
                return (
                  <div key={item.path} onClick={() => navigate(item.path)}
                    style={{
                      ...
                      background: isActive ? 'rgba(0,200,150,0.18)' : 'transparent',
      ...
      {/* User */}
      <div style={{ padding: '12px', borderTop: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.12)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
          <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: '#00c896', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', color: '#fff', flexShrink: 0 }}>
            {session?.user?.email?.[0]?.toUpperCase() || 'U'}
          </div>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{session?.user?.email || ''}</div>
        </div>
        <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '5px', padding: '5px 10px', fontSize: '12px', color: 'rgba(255,255,255,0.45)', cursor: 'pointer', width: '100%', textAlign: 'left' }}>Sign out</button>
      </div>
    </div>
  );
}
